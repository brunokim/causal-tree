...
export class SitesController {
  ...
  makeCheckbox(action, localId, remoteId, isDisabled, isChecked) {
    return $("<input>", { type: "checkbox" })
      ...
      .on("change", (evt) =>
        this.handleCheckboxChange(action, localId, remoteId, evt.target.checked)
      );
  }
  ...
  handleCheckboxChange(action, localId, remoteId, isChecked) {
    ...
    let remoteCheckbox = $(
      `tr[data-action='${oppositeAction}'] input[data-local='${remoteId}'][data-remote='${localId}']`,
      this.container
    );
  ...
  renderSyncArea(child) {
    ...
    for (let crdt of this.crdts) {
      ...
      let checkSend = this.makeCheckbox(
        "send",
        child.id,
        crdt.id,
        isSelf,
      ...
      let checkRecv = this.makeCheckbox(
        "recv",
        child.id,
        crdt.id,
        isSelf,
